...
6. TTSに読み上げさせるため、読み方が曖昧な漢字（例: 「辛い」は「からい」とも「つらい」とも読める）だけは、半角スペースで挟んだ" ひらがな "の形式で書くこと。TTSにとってかなり読みにくい漢字（例: 「今朝」→「けさ」、「絆創膏」→「ばんそうこう」）も半角スペースで挟んだ" ひらがな "の形式で書くこと。そのまま読めそうな漢字は漢字のままでいい。
