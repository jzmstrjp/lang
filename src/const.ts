 ...
 */
export const TTS_READING_RULES = `- japaneseReplyはTTSに読み上げさせるため、読み方が曖昧な漢字（例: 「辛く」は「からく」とも「つらく」とも読める）だけは、ひらがなで書いてほしい。半角スペースで挟んだ" からく "の形式で書くこと。TTSにとってかなり読みにくい漢字（例: 「今朝」→「けさ」、「絆創膏」→「ばんそうこう」）も半角スペースで挟んだ" ひらがな "の形式で書くこと。そのまま読めそうな漢字は漢字のままでいい。（例: "報告書は けさ 提出済みです。"）japaneseSentenceは読み上げないため、漢字のままでいい。`;
