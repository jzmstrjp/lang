...
export const ENGLISH_REPLY_PROMPT_RULES = `受信者の返答。englishSentenceに対する、要点を押さえつつできるだけ短い回答であること。目安は10単語以内。englishReplyを読めばenglishSentenceの内容が想像できるように具体的に言及すること。できれば最初に感動詞や相槌が欲しい。
  ...
  - 「こう返答したってことは、きっとこう話しかけられたんだろうな」と推測できるような内容にすること。ただしenglishSentenceの内容をそのままオウム返ししたenglishReplyは禁止する。englishSentenceの主題となる単語を1つだけenglishReplyにも含める程度が良い。
 ...
 *   - scripts/create-problems.ts createJapaneseConversation
 *   - src/app/api/admin/problems/regenerate-reply/route.ts (japaneseReply 翻訳)
 *   - src/app/api/admin/problems/improve-translation/route.ts (翻訳改善)
 */
...
/**
 * TTS（機械音声）読み上げ向けの共通ルール。
 * 翻訳結果を音声合成にかけるすべてのプロンプトで適用する。
 */
export const TTS_READING_RULES = `- 機械音声で読み上げるための日本語文なので、括弧書きは含めないこと。最後は「。」または「？」で終わること。
- TTSに読み上げさせるため、読み方が曖昧な漢字（例: 「辛く」は「からく」とも「つらく」とも読める）だけは、半角スペースで挟んだ" からく "の形式で書くこと。TTSにとってかなり読みにくい漢字（例: 「今朝」→「けさ」、「絆創膏」→「ばんそうこう」）も半角スペースで挟んだ" ひらがな "の形式で書くこと。そのまま読めそうな漢字は漢字のままでいい。`;
