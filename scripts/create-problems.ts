...
- sender: 送信者の情報（こちらが「${value}」というワードを使用する人物）
  - why: なぜこの発言をするのか、その動機や目的（最大20文字程度）「${value}」という言葉から必然的に導かれる自然な動機を設定してください。できるだけシンプルにしてください。（例: ワードが"good morning"なら「朝起きたので、挨拶をしたい」）
...
【要件】
- sender: 送信者の情報（こちらが「${value}」というワードを使用する人物）
  - why: なぜこの発言をするのか、その動機や目的（最大30文字程度）「${value}」という言葉から必然的に導かれる自然な動機を設定してください。（例: ワードが"good morning"なら「教室に入ったら友人を見つけたので、朝の挨拶をしたい」、ワードが"remote work"なら「来月からリモートワークが可能になるということを部下に伝えたい」）
