...
1. englishSentence: 送信者が話す英文。「${sceneDraft.word}」という表現を必ず使用すること。この文だけ読めば、文脈を知らなくても意図や状況が分かるような文が好ましい。
   ...
   - S・V・O・C・Mはそれぞれ具体的な英単語に置き換えること。（Sなら主語、Vなら動詞、Oなら目的語、Cなら補語、Mなら修飾語）
2. englishReply: ${ENGLISH_REPLY_PROMPT_RULES}
    ...
    ? `\`\`\`json
{
  "word": "I'm thirsty",
  "sender": {
    "why": "喉が渇いたので、飲み物を買いたいということを伝えたい",
    "role": "友人",
    "gender": "male",
    "where": "コンビニの前"
  },
    ...
    : `\`\`\`json
{
  "word": "remote work",
  "sender": {
    "why": "来月からリモートワークが可能になるということを部下に伝えたい",
    "role": "上司",
    "gender": "male",
    "where": "オフィスの自席"
  },
...
- sender: 送信者の情報（こちらが「${value}」というワードを使用する人物）
  - why: なぜこの発言をするのか、その動機や目的（最大20文字程度）「${value}」という言葉から自然に連想される、よくある動機を設定してください。できるだけシンプルにしてください。（例: ワードが"good morning"なら「朝起きたので、挨拶をしたい」）
...
【要件】
- sender: 送信者の情報（こちらが「${value}」というワードを使用する人物）
  - why: なぜこの発言をするのか、その動機や目的（最大30文字程度）「${value}」という言葉から自然に連想される、よくある動機を設定してください。（例: ワードが"good morning"なら「教室に入ったら友人を見つけたので、朝の挨拶をしたい」、ワードが"remote work"なら「来月からリモートワークが可能になるということを部下に伝えたい」）
