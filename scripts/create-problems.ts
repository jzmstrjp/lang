...
4. 必要そうであれば、1コマ目と2コマ目で「何が起こらないか」も簡潔に書くこと。(例: 男性は塩を持っていない、まだコーヒーは席に届いていない、テーブルには何もない)
...
【注意】whyとplaceが矛盾しないようにしてください。「今日は仕事を休むことを上司に伝えたい」シーンであれば「オフィス」で「対面」で伝えるのは、すでに出勤しているように見えるため不自然で矛盾しています。このシーンであれば「自宅」から「オフィス」に「電話」で伝えるのが自然です。
`;
...
- sender: 送信者の情報（こちらが「${value}」というワードを使用する人物）
  - why: なぜこの発言をするのか（15文字程度）「${value}」という言葉から必然的に導かれる自然な動機を設定してください。できるだけシンプルにしてください。（例: ワードが"good morning"なら「朝起きたので、挨拶をしたい」）
...
【要件】
- sender: 送信者の情報（こちらが「${value}」というワードを使用する人物）
  - why: なぜこの発言をするのか（20文字程度）「${value}」という言葉から必然的に導かれる自然な動機を設定してください。
