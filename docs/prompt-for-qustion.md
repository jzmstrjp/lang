...
type SeedProblemData = {
   ...
   * senderのセリフ。
   *
   * できればビジネス系でなく私生活系のセリフを生成してほしい。ただし指定された語彙（後述の「○問目: ××」で示されるワード）がビジネス系のワードであれば、ビジネス系のセリフを生成してください。
   *
   ...
   * englishSentenceとenglishReplyの内容から、電話での会話なのか対面での会話なのかビデオ会議なのかを判断して明記してください。音声で読み上げるので「メール」とか「チャット」という場面は禁止します。
   * 各登場人物が「まだやっていないこと」も明記してほしい。（例: receiverはまだ何も注文していない）
   *
   * 不自然な状況は、やめてください。
   * 悪い例:
   * - 待ち合わせに遅れそうなことを、対面で伝えている
   * 良い例:
   * - 待ち合わせに遅れそうなことを、電話で伝えている
   *
