...
- incorrectOptions
  - incorrectOptionsの3つのセリフの文字数はjapaneseSentenceと同じ文字数であること。
...
type SeedProblemData = {
   ...
   *
   * japaneseSentenceが疑問文なら、incorrectOptionsの各文も疑問文にすること。そうしないと文体で正解が推測されてしまう。
   * japaneseSentenceが平叙文なら、incorrectOptionsの各文も平叙文にすること。そうしないと文体で正解が推測されてしまう。
   * japaneseSentenceが命令文なら、incorrectOptionsの各文も命令文にすること。そうしないと文体で正解が推測されてしまう。
   * japaneseSentenceが勧誘文なら、incorrectOptionsの各文も勧誘文にすること。そうしないと文体で正解が推測されてしまう。
   *
   * incorrectOptionsの文頭が「ちなみに」から始まる問題ばかり生成するのとかは最悪です。「ちなみに、から始まるのは不正解だな」ってユーザーに推測されてしまいます。だからランダム性を高めてください。問題ごとに変えてください。
   * 悪い例: どの問題も「ちなみに」から始まるincorrectOptionばかり、どの問題も「実は」から始まるincorrectOptionばかり
   *
   * 3つのセリフの文字数はjapaneseSentenceと同じ文字数であること。
   * incorrectOptions の各文は、必ず異なる語から始めてください。同じ語で始まる文は禁止です。
...
- incorrectOptionsの3つのセリフの文字数はjapaneseSentenceと同じ文字数であること。
- incorrectOptions の各文は、必ず異なる語から始めてください。同じ語で始まる文は禁止です。
- JSONの全ての属性に「ルールを守れている根拠」をコメントとして書いてください。
